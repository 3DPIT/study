...
export function useSetUserField<K extends keyof User>(
  key: K
): ((value: User[K]) => void) {
  const setUser = useUserStore((state) => state.setUser);
  const [value, setValue] = useState<User[K]>(() => {
    const initialState = useUserStore.getState().user[key];
    return initialState;
  });
  ...
  useEffect(() => {
    const unsubscribe = useUserStore.subscribe(
      (newState) => {
        if (newState.user[key] !== value) {
          setValue(newState.user[key]);
        }
      },
...
export default function UserForm() {
  const setName = useSetUserField('name');
  const setEmail = useSetUserField('email');
